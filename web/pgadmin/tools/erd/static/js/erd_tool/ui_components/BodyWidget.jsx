...
export default class BodyWidget extends React.Component {
  ...
  handleAxiosCatch(err) {
	...
	async componentDidMount() {
		...
		try {
			this.setLoading('Initializing connection');
			let response = await this.initConnection();
			this.setLoading('Fetching required data');
			response = await this.loadPrequisiteData();
			if(this.props.params.gen) {
				this.setLoading('Fetching tables data');
				response = await this.loadTablesData();
			}
	...
	saveFile(fileName) {
		axios.post(url_for('sqleditor.save_file'), {
			'file_name': decodeURI(fileName),
			'file_content': JSON.stringify(this.diagram.serialize())
		}).then((res)=>{
    ...
    let scriptHeader = gettext('-- This script was generated by a beta version of the ERD tool in pgAdmin 4. \n');
		...
		this.setLoading('Preparing the SQL');
	...
	onImageClick() {
		...
		this.setLoading('Preparing the image...');
	...
	render() {
		return (
			...
			<ToolBar id="btn-toolbar">
				<ButtonGroup>
					<IconButton id="open-file" icon="far fa-folder-open" onClick={this.onLoadDiagram} title="Load from file"
						shortcut={this.state.preferences.open_project}/>
					<IconButton id="save-erd" icon="far fa-save" onClick={()=>{this.onSaveDiagram()}} title="Save project"
						shortcut={this.state.preferences.save_project} disabled={!this.state.dirty}/>
					<IconButton id="save-as-erd" icon="far fa-share-square" onClick={this.onSaveAsDiagram} title="Save as"
						shortcut={this.state.preferences.save_project_as}/>
				</ButtonGroup>
				<ButtonGroup>
					<IconButton id="save-sql" icon="far fa-file-code" onClick={this.onSQLClick} title="Generate SQL"
						shortcut={this.state.preferences.generate_sql}/>
					<IconButton id="save-image" icon="far fa-file-image" onClick={this.onImageClick} title="Download as image"
						shortcut={this.state.preferences.download_image}/>

					{/* <ReactToPrint trigger={() => {
						return <IconButton id="save-image" icon="far fa-file-image" title="Download as image"
						shortcut={this.state.preferences.download_image} />
					}} content={() => this.canvasRef.current}/> */}
				</ButtonGroup>
				<ButtonGroup>
					<IconButton id="add-node" icon="far fa-plus-square" onClick={this.onAddNewNode} title="Add table"
						shortcut={this.state.preferences.add_table}/>
					<IconButton id="edit-node" icon="fa fa-pencil-alt" onClick={this.onEditNode} title="Edit table"
						shortcut={this.state.preferences.edit_table} disabled={this.state.no_node_selected}/>
					<IconButton id="clone-node" icon="far fa-clone" onClick={this.onCloneNode} title="Clone table"
						shortcut={this.state.preferences.clone_table} disabled={this.state.no_node_selected}/>
					<IconButton id="delete-node" icon="far fa-trash-alt" onClick={this.onDeleteNode} title="Drop table/link"
						shortcut={this.state.preferences.drop_table} disabled={this.state.no_node_selected && this.state.no_link_selected}/>
				</ButtonGroup>
				<ButtonGroup>
					<IconButton id="add-note" icon="far fa-sticky-note" onClick={this.onNoteClick} title="Add/Edit note"
						shortcut={this.state.preferences.add_edit_note} disabled={this.state.no_node_selected}/>
					<IconButton id="add-onetomany" text="1M" onClick={this.onOneToManyClick} title="One-to-Many link"
						shortcut={this.state.preferences.one_to_many} disabled={this.state.no_node_selected}/>
					<IconButton id="add-manytomany" text="MM" onClick={this.onManyToManyClick} title="Many-to-Many link"
						shortcut={this.state.preferences.many_to_many} disabled={this.state.no_node_selected}/>
				</ButtonGroup>
				<ButtonGroup>
					<IconButton id="auto-align" icon="fa fa-magic" onClick={this.onAutoDistribute} title="Auto align"
				...
				<ButtonGroup>
					<IconButton id="zoom-to-fit" icon="fa fa-compress" onClick={this.diagram.zoomToFit} title="Zoom to fit"
						shortcut={this.state.preferences.zoom_to_fit}/>
					<IconButton id="zoom-in" icon="fa fa-search-plus" onClick={this.diagram.zoomIn} title="Zoom in"
						shortcut={this.state.preferences.zoom_in}/>
					<IconButton id="zoom-out" icon="fa fa-search-minus" onClick={this.diagram.zoomOut} title="Zoom out"
						shortcut={this.state.preferences.zoom_out}/>
				</ButtonGroup>
				<ButtonGroup>
					<IconButton id="help" icon="fa fa-question" onClick={()=>{}} title="Help" />
				</ButtonGroup>
			</ToolBar>
			...
			{/* <BottomBar message="the bottom bar...."></BottomBar> */}
			</>
		);
	}
}
